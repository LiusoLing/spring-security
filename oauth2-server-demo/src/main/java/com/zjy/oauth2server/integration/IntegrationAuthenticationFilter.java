...
public class IntegrationAuthenticationFilter extends GenericFilterBean {
     ...
     */
    public void setExceptionResponse(HttpServletResponse response, Exception exception) throws IOException {
        response.setStatus(HttpServletResponse.SC_CREATED);
        response.setContentType("application/json;charset=UTF-8");

        try {
            PrintWriter out = response.getWriter();
            out.write(JSONUtil.toJsonStr(Result.failed(exception.getLocalizedMessage())));
        } catch (IOException e) {
